...
# Config Hikari (Sony Xperia Acro S) Kernel
cp arch/arm/configs/nova_hikari_defconfig .config;
make ARCH=arm CROSS_COMPILE=$toolchain oldconfig;

# Clean the previous kernel leftovers
make clean;
...
cd zip-format;
zip -r NovaKernel-Hikari.zip ./META-INF kernel.elf;

# Move Kernel to shared folder
cp NovaKernel-Hikari.zip /media/sf_Desktop;

# Remove leftovers in zip-format folder
rm -f $kernel_dir/zip-format/kernel.elf;
rm -f $kernel_dir/zip-format/NovaKernel-Hikari.zip;
